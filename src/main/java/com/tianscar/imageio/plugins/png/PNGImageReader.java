...
class PNGImageDataEnumeration implements Enumeration<InputStream> {
    ...
    public PNGImageDataEnumeration(ImageInputStream stream, boolean fdAT)
        ...
        this.fdAT = fdAT;
        if (fdAT) stream.readInt(); // skip sequence number
    }
            ...
            try {
                while (true) {
                    ...
                    if (chunkType == acTL_TYPE) {
                        ...
                        if (imageStartPosition == -1L) animContainsIDAT = true;
                        parse_fcTL_chunk();
                    }
...
"Encountered unknown chunk with critical bit set!");
    ...
    private void readImage(int imageIndex, ImageReadParam param) throws IIOException {
        ...
        if (param != null && ((PNGImageReadParam) param).isForceReadIDAT()) isAnimated = false;

        int width, height;
        if (isAnimated) {
            PNGMetadata metadata = frameMetadata.get(imageIndex);
            if (metadata == null) throw new IIOException("Frame metadata not exist!");
    ...
    public int getWidth(int imageIndex) throws IIOException {
        ...
        if (isAnimated) {
            PNGMetadata metadata = frameMetadata.get(imageIndex);
            if (metadata == null) throw new IIOException("No image metadata exist this index: " + imageIndex);
    ...
    public IIOMetadata getImageMetadata(int imageIndex) throws IIOException {
        ...
        if (isAnimated) {
            PNGMetadata metadata = frameMetadata.get(imageIndex);
            if (metadata == null) throw new IIOException("No image metadata exist for this index: " + imageIndex);
            return metadata;
        } else if (imageIndex != 0) {
            throw new IndexOutOfBoundsException("imageIndex != 0!");
        }
